...
def run_udev_rule_commands():
    prefix = subprocess.check_output(['ros2', 'pkg', 'prefix', 'tecgihan_driver'], text=True).strip()
    print(prefix)
    print("Copying udev rules")
    cp_command = ['sudo', 'cp', f'{prefix}/share/tecgihan_driver/debian/udev', '/etc/udev/rules.d/90-tecgihan.rules']
