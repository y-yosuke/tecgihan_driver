...
def format_google_docstring(docstring):
    ...
    for line in lines:
        ...
        one_liner_match = re.match(r"^(Note|Notes|Warning|Warnings):\s*(.+)", stripped)
        if one_liner_match:
            flush_section()
        ...
        section_match = re.match(r"^(Args|Arguments|Parameters|Returns|Yields|Raises|Examples|Note|Notes):$", stripped)
        if section_match:
            flush_section()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate Markdown API documentation (Google style docstring).")
    parser.add_argument("-i", "--input-dir", default=".", help="Input directory containing Python files (default: current directory)")
    parser.add_argument("-o", "--output-dir", default=".", help="Directory to output Markdown files (default: current directory)")
    parser.add_argument("-c", "--clean-pycache", action="store_true", help="Delete __pycache__ directories after processing")
